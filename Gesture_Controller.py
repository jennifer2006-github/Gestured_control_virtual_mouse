...
def download_hand_landmarker_model():
    """Download the hand landmarker model from Google"""
...
# Get model path
HAND_MODEL_PATH = download_hand_landmarker_model()

class DrawingUtils:
    @staticmethod
    def draw_landmarks(image, landmarks, connections=None, landmark_drawing_spec=None):
        h, w, c = image.shape
        ...
        for start,end in connections:
            x1=int(landmark_list[start].x*w)
            y1=int(landmark_list[start].y*h)

            x2=int(landmark_list[end].x*w)
            y2=int(landmark_list[end].y*h)
...
class LandmarkWrapper:
    """Wrapper to provide old mp.solutions API compatibility for landmarks"""
    def __init__(self, landmark_list):
        self.landmark = landmark_list if landmark_list else []
    ...
    def _init_detector(self):
        try:
            if HAND_MODEL_PATH:
                ...
                raise Exception("Hand landmarker model not available")
        except Exception as e:
            print(f"Warning: HandLandmarker initialization failed: {e}")
            print("Hand detection may not work properly")
            self.detector = None
    
    def process(self, image):
        """Process image and return results"""
        if not self.detector:
            # Return empty results if detector not available
            class EmptyResults:
        ...
        try:
            ...
            # Wrap results to provide compatibility with old API
            class WrappedResults:
                def __init__(self, hand_landmarks, handedness):
                    self.multi_hand_landmarks = hand_landmarks if hand_landmarks else []
                    self.multi_handedness = handedness if handedness else []
            
            return WrappedResults(results.hand_landmarks, results.handedness)
        except Exception as e:
...
# Create a wrapper class that mimics mp.solutions.hands
class HandsWrapper:
    HAND_CONNECTIONS = None
    
    def Hands(self, max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5):
        """Create a hands detector with context manager support"""
        return HandsDetector(max_num_hands, min_detection_confidence, min_tracking_confidence)

mp_hands = HandsWrapper()
...
class GestureController:
    ...
    def __init__(self):
        ...
        cap_holder = [None]  # Use list to allow modification in nested function
        error_holder = [None]
        
        def try_camera_init():
            try:
                # Try DirectShow first (better on Windows)
                ...
                if cap.isOpened():
                    ...
                    error_holder[0] = "Could not open camera with any backend"
            except Exception as e:
                error_holder[0] = str(e)
        
        # Initialize camera in a thread with timeout
        init_thread = threading.Thread(target=try_camera_init, daemon=True)
        init_thread.start()
        init_thread.join(timeout=3.0)  # Wait max 3 seconds
        
        if cap_holder[0] is None:
            if error_holder[0]:
                print(f"ERROR: {error_holder[0]}")
            else:
                print("ERROR: Camera initialization timed out (possible driver issue)")
            raise RuntimeError("Camera initialization failed")
        
        GestureController.cap = cap_holder[0]
        ...
        print(f"[OK] Camera initialized: {int(GestureController.CAM_WIDTH)}x{int(GestureController.CAM_HEIGHT)}")
    ...
    def classify_hands(results):
        ...
        # results.multi_hand_landmarks is a list of landmark lists
        # results.multi_handedness is a list of handedness objects
        
        if len(results.multi_handedness) > 0:
            # First hand
            try:
                handedness = results.multi_handedness[0]
                # handedness is a Category object with display_name and score
                label = handedness.display_name if hasattr(handedness, 'display_name') else 'Unknown'
                
                if label == 'Right':
                    right = LandmarkWrapper(results.multi_hand_landmarks[0])
                else:
                    left = LandmarkWrapper(results.multi_hand_landmarks[0])
            except Exception as e:
                pass
        ...
        if len(results.multi_handedness) > 1:
            # Second hand
            try:
                handedness = results.multi_handedness[1]
                label = handedness.display_name if hasattr(handedness, 'display_name') else 'Unknown'
                
                if label == 'Right':
                    right = LandmarkWrapper(results.multi_hand_landmarks[1])
                else:
                    left = LandmarkWrapper(results.multi_hand_landmarks[1])
            except Exception as e:
                pass
